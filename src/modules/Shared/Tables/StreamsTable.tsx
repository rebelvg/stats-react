...
const tableConfigTemplate = [
  ...
  },
  {
    ...
    accessor: 'ip',
    Cell: (props) => {
      if (!props.original.location?.api)
        return `${props.value} (${props.original.userName})`;

      return `${props.value} (${props.original.location?.api?.countryCode}/${props.original.location?.api?.city}) (${props.original.userName})`;
    },
  },
  {
    Header: 'Connections',
